"""2D circular blast wave simulation using CFDGrid."""
import time
import numpy as np
...
def blast_ic_2d(x, y, p_high=1.0, p_low=0.1, r0=0.1, center_x=0.5, center_y=0.5):
...
def run_blast_wave(tEnd=0.3, nx=100, ny=100, plot=True):
    """Run 2D circular blast wave simulation.
    ...
    if plot:
        ...
        ax.set_title('2D Circular Blast Wave')
        ax.set_xlabel('x')
        ax.set_ylabel('y')
    ...
    while t < tEnd:
        ...
        if plot and it % 5 == 0:
            im.set_data(p)
            ax.set_title(f'2D Circular Blast Wave (t={t:.3f})')
            plt.pause(0.001)
...
if __name__ == "__main__":
    run_blast_wave(tEnd=0.3, nx=100, ny=100, plot=True)
